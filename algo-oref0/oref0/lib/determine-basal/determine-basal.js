...
// Set variables required for evaluating error conditions
    var rT = { reason: "" }; //short for requestedTemp
...
// Use minGuardBG to prevent overdosing in hypo-risk situations
    ...
    if (enableSMB && minGuardBG < threshold) {
        console.error("minGuardBG",convert_bg(minGuardBG, profile),"projected below", convert_bg(threshold, profile));
        // 🔥 극도로 적극적인 SMB: minGuardBG 체크를 완화 (매우 낮은 경우에만 비활성화)
        if (minGuardBG < threshold - 20) { // 원래는 threshold, 이제는 threshold-20
            console.error("- disabling SMB due to very low minGuardBG");
            enableSMB = false;
        } else {
            console.error("- allowing SMB despite low minGuardBG (AGGRESSIVE MODE)");
        }
    }
...
var maxDelta_bg_threshold;
    if (typeof profile.maxDelta_bg_threshold === 'undefined') {
        ...
        // 🔧 Remove the 0.3 cap to allow higher thresholds for high BG SMB
        maxDelta_bg_threshold = profile.maxDelta_bg_threshold;
    }
...
// Begin core dosing logic: check for situations requiring low or high temps, and return appropriate temp after first match
    ...
    if (iob_data.iob > max_iob) {
        ...
        if (microBolusAllowed && enableSMB && bg > threshold) {
            ...
            var microBolus = Math.floor(Math.min(insulinReq,maxBolus)*roundSMBTo)/roundSMBTo; // 🔥 전체 insulinReq 사용!
            // calculate a long enough zero temp to eventually correct back up to target
            ...
            if (profile.SMBInterval) {
                // 🔥 SMB 간격 제한을 거의 제거 (최소 0.1분까지 허용)
                SMBInterval = Math.min(10,Math.max(0.1,profile.SMBInterval));
            }
            ...
            // 🔥 극도로 적극적인 SMB: 간격 제한을 거의 무시
            if (lastBolusAge > SMBInterval || SMBInterval <= 0.5) {
                if (microBolus > 0) {
                    rT.units = microBolus;
                    rT.reason += "🔥 AGGRESSIVE Microbolusing " + microBolus + "U. ";
                }
            } else {
